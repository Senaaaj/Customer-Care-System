...
<%@ page import="Feedback.feedbackController" %>
<%@ page import="Feedback.FeedbackModel" %>
<%@ page import="java.util.*" %>

<%
    String fid = request.getParameter("fid");
    List<FeedbackModel> feedbackData = feedbackController.getById(fid);
    FeedbackModel fb = null;

    String fname = "", fphone = "", femail = "", connection_ = "", branch = "";
    String satisfaction = "", recommendation = "", comments = "";

    if (!feedbackData.isEmpty()) {
        fb = feedbackData.get(0);
        fname = fb.getFname();
        fphone = fb.getFphone();
        femail = fb.getFemail();
        connection_ = fb.getConnection_();
        branch = fb.getBranch();
        satisfaction = fb.getSatisfaction();
...
<header>
  ...
  <nav>
    ...
    <form id="form" action="feedbackUpdateServlet" method="POST">
    ...
    <h2>Customer Feedback Form</h2>
      ...
      <input type="tel" name="fphone" value="<%= fphone %>" required>

      <label for="email">Email *</label>
     <input type="email" name="femail" value="<%= femail %>" required>

      <label for="connection">Feedback Type *</label>
      <select id="connection" name="connection_" required>
       <option value="<%= connection_ %>" selected><%= connection_ %></option>
      ...
      <select id="satisfaction" name="satisfaction" required>
         <option value="<%= satisfaction %>" selected><%= satisfaction %></option>
